	...
	while(izbor != 'K' && izbor != 'k')
	{
		...
		case '4':
			...
			if (redni_broj == -1)
				...
				printf("\nOsoba je: %s %s %d", popis[redni_broj].ime, popis[redni_broj].prezime, popis[redni_broj].godina);
			break;
		case '5':
...
void u_dat(int n, struct osoba x[])
{
	...
	{
		for(i = 0; i<n ; i++)
			fprintf(dat, "\n%s %s %d", x[i].ime, x[i].prezime, x[i].godina);
		fclose(dat);
	}
...
int iz_dat(int n, struct osoba x[])
{
	...
	else
	{
		...
		{
			fscanf(dat, " %s %s %d", x[i].ime, x[i].prezime, &x[i].godina);
			i ++;
		}
		fclose(dat);
